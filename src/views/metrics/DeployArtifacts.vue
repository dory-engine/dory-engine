<template>
  <page-card :name="$vuetify.lang.t('$vuetify.lang_menu_metrics_deploy_artifacts')">
    <v-card-text>
      <v-tabs v-model="tabValue">
        <v-tab>{{$vuetify.lang.t('$vuetify.lang_view_metrics_deploy_artifacts_count')}}</v-tab>
        <v-tab @change="change2()">{{$vuetify.lang.t('$vuetify.lang_view_metrics_deploy_artifacts_duration')}}</v-tab>
        <v-tab @change="change3()">{{$vuetify.lang.t('$vuetify.lang_view_metrics_deploy_artifacts_weekly')}}</v-tab>
        <v-tab @change="change4()">{{$vuetify.lang.t('$vuetify.lang_view_metrics_deploy_artifacts_status_rate')}}</v-tab>
      </v-tabs>
...
export default {
  ...
  created () {
    const vm = this
    request.get('/metrics/deployArtifacts/options').then(response => {
  ...
  mounted () {
    ...
    request.post('/metrics/deployArtifacts/count', vm.chartParams).then(response => {
      vm.config = {
        type: 'line',
        data: {
  ...
  methods: {
    ...
    getEcharts2 () {
      ...
      if(vm.chartParams2.timeRangeDays == 0 && vm.dates2.length == 0){
        ...
        request.post('/metrics/deployArtifacts/duration', vm.chartParams2).then(response => {
          vm.config2 = {
            type: 'line',
            data: {
              labels: response.data.labels,
    ...
    getEcharts3 () {
      ...
      if(vm.chartParams3.timeRangeDays == 0 && vm.dates3.length == 0){
        ...
        request.post('/metrics/deployArtifacts/countWeekly', vm.chartParams3).then(response => {
          vm.config3 = {
            type: 'line',
            data: {
              labels: response.data.labels,
    ...
    getEcharts4 () {
      ...
      if(vm.chartParams4.timeRangeDays == 0 && vm.dates4.length == 0){
        ...
        vm.statusResultsList.map((item,index) => {
          ...
          request.post(`/metrics/deployArtifacts/status/${item}`, vm.chartParams4).then(response => {
            vm.config4[index] = {
              type: 'line',
              data: {
                labels: response.data.labels,
